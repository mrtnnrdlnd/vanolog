<script lang="ts">
    ...
    // RÄKNA LIVE: Hämtar data för en kolumn och räknar ut medelvärde direkt
    function getStatForColumn(ds: Dataset, colIndex: number): number | null {
        ...
        // Loopa bara de få raderna i denna kolumn (t.ex. 7 st)
    ...
    <g class="graph-layer">
        ...
        {#each layoutStore.visuals.lines as line (line.id)}
            ...
            {#if line.graphMode === 'all'}
                {#if line.showLine}
                    <path d={getDetailedPath(line)} fill="none" stroke={line.color} stroke-width={Math.max(1, line.width - 1)} stroke-linejoin="round" vector-effect="non-scaling-stroke" />
                {/if}

            {:else if line.graphType === 'line'}
                {#if line.showLine}
                    <path d={getSummaryPath(line)} fill="none" stroke={line.color} stroke-width={line.width} stroke-linecap="round" stroke-linejoin="round" />
                {/if}
                ...
                {#if line.showMarkers}
                    {#each {length: (renderEnd - renderStart + 3)} as _, offset}
                        ...
                        {#if val !== null}
                            ...
                            {@const y = layoutStore.getY(val)}
                            <circle cx={xLine} cy={y} r={line.markerSize} fill="#fff" stroke={line.color} stroke-width={line.width} opacity={line.markerOpacity} />
                        {/if}
                    {/each}
                {/if}
            ...
            {:else}
                {#each {length: (renderEnd - renderStart + 3)} as _, offset}
                    ...
                    {#if val !== null}
                        ...
                        <rect x={xBase + 2} y={y} width={CONFIG.stride - 4} height={yBottom - y} fill={line.color} rx="2" opacity="0.8" />
                    {/if}
                {/each}
            {/if}
