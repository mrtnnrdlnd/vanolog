<script lang="ts">
    ...
    function toggleDatasetEdit(id: string) { editingDatasetId = editingDatasetId === id ? null : id; }
...
{#snippet toggleRow(label: string, checked: boolean, onChange: (v: boolean) => void)}
    <div class="setting-row">
        ...
        <button class="toggle-switch {checked ? 'active' : ''}" onclick={() => onChange(!checked)}>
            <div class="toggle-knob"></div>
        </button>
    </div>
{/snippet}

{#snippet selector(label: string, options: any[], currentValue: any, onChange: (v: any) => void, layout = 'flex')}
    <div class="setting-group">
        <span class="group-label">{label}</span>
        <div class="selector-container {layout}">
            {#each options as opt}
                <button class:active={currentValue === opt.value} class:full-width={opt.fullWidth} onclick={() => onChange(opt.value)}>
                    {opt.label}
                </button>
            {/each}
        </div>
...
{#if isOpen}
    <div class="settings-backdrop" style:top="{CONFIG.titleBarHeight}px" onclick={close} role="button" tabindex="0" onkeydown={(e) => e.key === 'Escape' && close()} transition:fade={{ duration: 200 }}>
        <div class="settings-panel" onclick={(e) => e.stopPropagation()} role="document" tabindex="0" onkeydown={(e) => e.stopPropagation()} transition:fly={TRANS_PANEL}>
            <div class="panel-scroll-area">
                
                {#snippet datasetsContent()}
                    <div class="content-stack">
                        <p class="help-text" style:text-align="left" style:margin-bottom="8px">Konfigurera grafer.</p>
                            ...
                            <div class="dataset-card">
                                <div class="dataset-row">
                                    <button class="expand-btn" onclick={() => toggleDatasetEdit(ds.id)}>
                                        ...
                                        <div style="display:flex; flex-direction:column; gap:2px;">
                                            ...
                                            <span class="ds-meta">{ds.graphType === 'line' ? 'Linje' : 'Stapel'} • {ds.graphMode.toUpperCase()}</span>
                                        </div>
                                    </button>
                                    <button class="toggle-switch {ds.isVisible ? 'active' : ''}" onclick={() => dataStore.toggleDataset(ds.id)}>
                                        <div class="toggle-knob"></div>
                                    </button>
                                </div>
                                {#if editingDatasetId === ds.id}
                                    <div class="dataset-controls" transition:slide={{duration: 200, easing: quintOut}}>
                                        ...
                                        <div class="control-section-label" style:margin-top="8px">UTSEENDE</div>
                                        ...
                                        <div class="control-row">
                                            ...
                                            <input type="range" min="0.5" max="8" step="0.5" bind:value={ds.width} class="width-slider" />
                                        </div>

                                        {#if ds.graphType === 'line'}
                                            {@render toggleRow("Visa Linje", ds.showLine, (v) => ds.showLine = v)}
                                            {@render toggleRow("Visa Markörer", ds.showMarkers, (v) => ds.showMarkers = v)}
                                            {#if ds.showMarkers}
                                                <div class="control-row" style="margin-top:6px">
                                                    <label>Markörstorlek ({ds.markerSize}px)</label>
                                                    <input type="range" min="1" max="10" step="0.5" bind:value={ds.markerSize} class="width-slider" />
                                                </div>
                                                <div class="control-row">
                                                    <label>Opacitet ({Math.round(ds.markerOpacity * 100)}%)</label>
                                                    <input type="range" min="0" max="1" step="0.1" bind:value={ds.markerOpacity} class="width-slider" />
                                                </div>
                                            {/if}
                                        {/if}

                                        <div class="action-row" style:margin-top="12px">
                                            <button class="action-btn copy" onclick={() => dataStore.cloneDataset(ds.id)} title="Duplicera"><span>📋</span> Duplicera</button>
                                            {#if dataStore.datasets.length > 1}
                                                <button class="action-btn delete" onclick={() => dataStore.removeDataset(ds.id)} title="Ta bort"><span>🗑️</span> Ta bort</button>
                                            {/if}
                                        </div>
                                    </div>
                                {/if}
                        ...
                        {#if layoutStore.showHeatmap}
                            <div class="setting-group">
                                ...
                                <div class="hue-wrapper">
                                    <input id="hue-slider" type="range" min="0" max="360" bind:value={layoutStore.heatmapHue} class="hue-slider" style:background={getGradient(layoutStore.darkMode)} />
